...
class DBase:
    ...
    def add_user(self, username):
        ...
        try:
            print(f"Проверяем файл: {user_file}")
            max_user_id = self.get_max_user_id(user_file)
            print(f"Максимальный user_id: {max_user_id}")
            new_user["user_id"] = str(max_user_id + 1)
            new_user["username"] = username
            new_user["key"] = self.generate_unique_key()
            print(f"Создаём пользователя: {new_user}")

            self.save_single_entry_to_csv("user", new_user)
            print("Пользователь успешно сохранён.")

            lot_file = f"{self.schema_name}/lot/1.csv"
            max_lot_id = self.get_max_lot_id(lot_file)
            print(f"Максимальный lot_id: {max_lot_id}")
            ...
            print("Лоты для пользователя успешно добавлены.")
...
class TradingBot:
    ...
    def trade(self):
        while self.is_running:
            ...
            quantity = round(random.uniform(80, 120))  
            order_type = random.choice(["buy"])

            print(f"Выставление ордера: {order_type} {quantity} на пару {pair_id}")
            result = self.place_order(pair_id, quantity, order_type)
